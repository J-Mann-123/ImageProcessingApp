...
it('Should expect edited-JohnWick.jpg to exist', () => {
  expect(
    fs_1.default.existsSync('src/thumbs/edited-JohnWick.jpg')
  ).toBeTruthy();
});
// this should pass only after npm run start has been run
it('Should return false for an invalid link', async () => {
  const response = await (0, isomorphic_fetch_1.default)(
    'http://localhost:3000/images/JohnWick.jpg'
  );
...
it('Should return false for an invalid link', async () => {
  const response = await (0, isomorphic_fetch_1.default)(
    'http://localhost:3000/thumbs/edited-JohnWick.jpg'
  );
  expect(response).toBeTruthy();
});
...
it('Should return false for an invalid link', async () => {
  const response = await (0, isomorphic_fetch_1.default)(
    'http://localhost:3000/resize?imageUrl=https://localhost:3000/images/JohnWick.jpg&width=340&height=380'
  );
  expect(response).toBeTruthy();
});
